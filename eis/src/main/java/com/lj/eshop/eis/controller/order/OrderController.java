...
public class OrderController extends BaseController{
	...
	@ResponseBody
	public ResponseDto createByCar(String cars,String addrCode,Boolean isInvoice, String invoiceTitle,String invoiceInfo,String remarks) {
		...
		for (String string : carss) {
			...
			if(shopCarDto==null){
				return ResponseDto.createResp(false, ResponseCode.CAR_NOT_EXIST.getCode(), ResponseCode.CAR_NOT_EXIST.getMsg(), null);
			}
			
			/*不是自己的购物车*/
			if(!shopCarDto.getMbrCode().equals(getLoginMemberCode())){
				return ResponseDto.createResp(false, ResponseCode.ACCESS_VALID.getCode(), ResponseCode.ACCESS_VALID.getMsg(), null);
			}
		...
		List<OrderDto> list= orderService.createByCar(shopCarDtos, addrsDto, isInvoice, invoiceTitle, invoiceInfo,remarks,province==null?"":province.getName(),city==null?"":city.getName(),area==null?"":area.getName());
	...
	public ResponseDto create(String skuCode,String shopCode,Integer cnt,String addrCode,Boolean isInvoice, 
		...
		logger.info("createByCar --> String invoiceInfo={}- start",invoiceInfo);
		if(StringUtils.isEmpty(skuCode)|| StringUtils.isEmpty(shopCode) || StringUtils.isEmpty(addrCode) || isInvoice==null ||cnt==null ||cnt==0){
			return ResponseDto.createResp(false, ResponseCode.PARAM_ERROR.getCode(), ResponseCode.PARAM_ERROR.getMsg(), null);
		}
		...
		OrderDto orderDto = orderService.createOrder(productSkuDto, shopCode, cnt, addrsDto, isInvoice, invoiceTitle, invoiceInfo,remarks,province==null?"":province.getName(),city==null?"":city.getName(),area==null?"":area.getName());
		return ResponseDto.successResp(orderDto);
	}
	/**
	 * 
	...
	public ResponseDto createByRank(String skuCode,String shopCode,Integer cnt,String addrCode, String remarks) {
		...
		//校验不为空
		if(StringUtils.isEmpty(skuCode)|| StringUtils.isEmpty(shopCode) || StringUtils.isEmpty(addrCode)  ||cnt==null ||cnt==0){
			return ResponseDto.createResp(false, ResponseCode.PARAM_ERROR.getCode(), ResponseCode.PARAM_ERROR.getMsg(), null);
		}
		...
		if(null==shopDto || null==shopDto.getRankCode()) {
			return ResponseDto.createResp(false, ResponseCode.MEMBER_RANK_APPLY_NOT_FOUND_BUY.getCode(), ResponseCode.MEMBER_RANK_APPLY_NOT_FOUND_BUY.getMsg(), null);
		}
		
		//特权过期
		if(shopDto.getRankExpireTime()==null) {
			return ResponseDto.createResp(false, ResponseCode.MEMBER_RANK_APPLY_EXTIRED.getCode(), ResponseCode.MEMBER_RANK_APPLY_EXTIRED.getMsg(), null);
		}
		...
		if(null==accountDto.getRankCashAmt() || totalSalePrice.compareTo(accountDto.getRankCashAmt())>0) {
			return ResponseDto.createResp(false, ResponseCode.MEMBER_RANK_APPLY_NOT_YET_AMT.getCode(), 
						ResponseCode.MEMBER_RANK_APPLY_NOT_YET_AMT.getMsg()+accountDto.getRankCashAmt()+"元",
					null);
		}
		...
		if(memberRankDto.getScale()>0) {
			...
			if(null!=list && list.size()>0) {
				if(list.size()>=memberRankDto.getScale()) {
					return ResponseDto.createResp(false, ResponseCode.MEMBER_RANK_APPLY_NOT_YET_TIMES.getCode(), ResponseCode.MEMBER_RANK_APPLY_NOT_YET_TIMES.getMsg(), null);
				} 
			}
		}
		...
		if(productSkuDto.getCnt()<cnt) {
			return ResponseDto.createResp(false, ResponseCode.MEMBER_RANK_APPLY_NOT_YET_STORAGE.getCode(), ResponseCode.MEMBER_RANK_APPLY_NOT_YET_STORAGE.getMsg(), null);
		}
		...
		//创建订单
		OrderDto orderDto = orderService.createOrder(productSkuDto, shopCode, cnt, addrsDto, false, null, null, remarks,province==null?"":province.getName(),city==null?"":city.getName(),area==null?"":area.getName());
		//流水
	...
	public ResponseDto receipt(String code) {
		logger.info("createByCar --> String code={}- start",code);
