...
public class ShopCarController extends BaseController{
	...
	public ResponseDto add(String shopCode,String productSkuCode,Integer cnt) {
		logger.info("add -->  - start");
			...
			parmShopDto.setCode(shopCode);
			ShopDto shopDto=shopService.findShop(parmShopDto);
			if(shopDto.getStatus().equals(ShopStatus.STOP.getValue())){
				return ResponseDto.createResp(false, ResponseCode.SHOP_UNNORMAL.getCode(), ResponseCode.SHOP_UNNORMAL.getMsg(), null);
			}
	...
	public ResponseDto getByIds(String[] codes) {
		logger.info("getByIds --> List<String> codes="+codes.length+") - start");
			...
			for (String string : codes) {
				...
				FindShopCarPage findShopCarPage = new FindShopCarPage();
				findShopCarPage.setParam(param);
				returnList.add(shopCarService.findShopCars(findShopCarPage).get(0));
			}
