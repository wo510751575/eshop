...
public class SmsCodeService {
	...
	public void send(SmsCodeSenderRequest request) {
		...
		logger.info("key:" + key);
		String validateCode =redisClient.get(key);
	...
	public String verify(SmsCodeVerifyRequest request) {
		...
		if(request.getSmsCode().equals(value))	{
			...
			if(request.isProcessFlag()) {
				...
				// 将随机校验码存入缓存，并设置过期时间
				redisClient.setex(PROCESS_CODE_KEY_PREFIX + request.getMobile() + request.getSmsCode(), request.getProcessExpired(), processCode);
			}
			redisClient.del(key);	// 验证通过，删除key，以便可以接受后续的验证码
			return processCode;
		} else {
