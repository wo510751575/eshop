...
import com.lj.eoms.dao.sys.MenuDao;
import com.lj.eoms.dao.sys.OfficeDao;
import com.lj.eoms.dao.sys.RoleDao;
import com.lj.eoms.dao.sys.UserDao;
...
public class UserUtils {
	...
	private static IMerchantService merchantService = SpringContextHolder.getBean(IMerchantService.class);
	/**
	 * 根据ID获取用户
	 * @param id
	 * @return 取不到返回null
	...
	public static User get(String id){
		...
		if (user ==  null){
			...
			user.setMerchant(findMerchantByUser(user));//设置电商商户信息 2017-8-25 hy
	 ...
	 * 
	 *
	 * 方法说明：根据用户所属公司ID找用户归属的电商商户信息.
	 *
	 * @param user
	 * @return
	 *
	 * @author lhy  2017年8月25日
	 *
	 */
	private static MerchantDto findMerchantByUser(User user){
		//根据公司名找商户
		MerchantDto m=null;
		FindMerchantPage page=new FindMerchantPage();
		MerchantDto param=new MerchantDto();
		param.setOfficeId(user.getCompany().getId());
		page.setParam(param);
		List<MerchantDto> rt=merchantService.findMerchants(page);
