...
public class EcShopServiceImpl implements IEcShopService { 
	...
	private static final Logger logger = LoggerFactory.getLogger(EcShopServiceImpl.class);
	...
	public AddShopReturn addShop(
		...
		try {
			...
			shop.setShopNoMerchant(addShop.getShopNoMerchant());//门店代码（商户自定义）
	...
	@Override
	public List<FindShopPageReturn> findShopNoByCode(FindShop findShop) {
		logger.debug("findShop(FindShop findShop={}) - start", findShop); //$NON-NLS-1$
		AssertUtils.notNull(findShop);
		List<FindShopPageReturn> findShopPageReturn=null;
		try {
