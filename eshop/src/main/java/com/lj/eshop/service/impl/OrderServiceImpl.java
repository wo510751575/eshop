...
import com.lj.eshop.constant.Global;
...
public class OrderServiceImpl implements IOrderService { 
	...
	public void addOrder(
			OrderDto orderDto) throws TsfaServiceException {
	...
	@Override
	public Page<OrderDto> findOrderPage(
			FindOrderPage findOrderPage)
			throws TsfaServiceException {
	...
	@Override
	public List<OrderDto> createByCar(List<ShopCarDto> shopCarDtos,AddrsDto addrsDto,boolean isInvoice, String invoiceTitle,String invoiceInfo,String remarks,String province,String city,String area) {
		...
		try {
			...
			if(shopCarDtos!=null && shopCarDtos.size()>0){
				...
				for (ShopCarDto shopCarDto : shopCarDtos) {
					...
					parmShopDto.setCode(shopCarDto.getShopCode());
					...
					/*根据店铺创建订单*/
					if(!shopCodeTmp.equals(shopCarDto.getShopCode())){
						/*赋值临时变量*/
						shopCodeTmp = shopCarDto.getShopCode();
						...
						orderNo = NoUtil.generateNo(NoUtil.JY);
					...
					/*C端售价*数量*/
					detailDto.setAmt(productSkuDto.getSalePrice().multiply(new BigDecimal(shopCarDto.getCnt())));
					detailDto.setSalePrice(productSkuDto.getSalePrice());				//C端单个售价
					BigDecimal disCount = new BigDecimal(supplyDto.getDiscountOff());	//供应商折扣（0-100的数字）
					detailDto.setOrgPrice(productSkuDto.getSalePrice().multiply(disCount).divide(new BigDecimal(100)).add(productSkuDto.getSalePrice()));					//小B进货单价（按供应商折扣计算）
					detailDto.setGapPrice(productSkuDto.getSalePrice().multiply(disCount).divide(new BigDecimal(100)));				//小B提成单价（按供应商折扣计算）
	...
	public void complete(OrderDto orderDto) {
		...
		try {
			...
			if (detailList.size()>0) {
				...
				while (localIterator.hasNext()) {
					...
					if ((item != null) && (item.getGapPrice() != null)&& (item.getCnt() != null)) {
						gapPrice = gapPrice.add(item.getGapPrice().multiply(new BigDecimal(item.getCnt()-item.getReturnCnt())));
					}
					/*计算订单金额*/
					if ((item != null) && (item.getAmt() != null)&& (item.getReturnCnt() != null)&& (item.getSalePrice() != null)) {
						amt = amt.add(item.getAmt().subtract(item.getSalePrice().multiply(new BigDecimal(item.getReturnCnt()))));
					}
			...
			parmShop.setCode(orderDto.getShopCode());
	...
	public void shipping(OrderDto orderDto,String expressNo,String expressName) {
		...
		try {
			...
			if(orderDto!=null && orderDto.getStatus().equals(OrderStatus.UNSHIPPED.getValue())){
				/*状态流转至待收货*/
				orderDto.setStatus(OrderStatus.SHIPPED.getValue());
				orderDto.setExpressNo(expressNo);
				orderDto.setExpressName(expressName);
				orderDto.setShippingTime(new Date());
				this.updateOrder(orderDto);
				/*消息通知	*/
				OrderDto paramOrder = new OrderDto();
				paramOrder.setCode(orderDto.getCode());
				OrderDto returnOrders = findOrder(paramOrder);
				sendMessageByOrder(returnOrders, MessageTemplate.C_SYS_NOT_PARAM_COMMODITY_SENDED);
			}
		}catch (TsfaServiceException e) {
			logger.error(e.getMessage(),e);
			throw e;
	...
	public void returns(OrderRetireDto orderRetireDto,String failReason) {
		...
		try {
			...
			if(orderRetireDto.getAuditStatus().equals(AuditStatus.SUCCESS.getValue())){
				...
				List<OrderRetireDetailDto> retireDetailList =orderRetireDetailService.findOrderRetireDetails(findOrderRetireDetailPage);
				...
				for (OrderRetireDetailDto orderRetireDetailDto : retireDetailList) {
					...
					if (orderDetailDto != null) {
						orderDetailDto.setReturnCnt(Integer.valueOf(orderDetailDto.getReturnCnt().intValue()+ orderRetireDetailDto.getCnt().intValue()));
					}
					...
					/*计算提成*/
					if ((orderDetailDto != null) && (orderDetailDto.getGapPrice() != null)&& (orderRetireDetailDto.getCnt() != null)) {
						gapPrice = gapPrice.add(orderDetailDto.getGapPrice().multiply(new BigDecimal(orderRetireDetailDto.getCnt())));
					}
	...
	@Override
	public OrderDto createOrder(ProductSkuDto paramSku,String shopCode,Integer cnt, AddrsDto addrsDto,
			boolean isInvoice, String invoiceTitle, String invoiceInfo,String remarks,String province,String city,String area) {
		...
		try {
			...
			paramShop.setCode(shopCode);
			ShopDto shopDto= shopService.findShop(paramShop);
			if(shopDto.getStatus().equals(ShopStatus.STOP.getValue())){
				logger.error("店铺已关闭！");
				throw new TsfaServiceException(ErrorCode.SHOP_STOP_ERROR,"店铺已关闭！");
			}
				...
				/*创建订单*/
				orderDto.setAmt(paramSku.getSalePrice().multiply(new BigDecimal(cnt)));
				...
				/*C端售价*数量*/
				detailDto.setAmt(paramSku.getSalePrice().multiply(new BigDecimal(cnt)));
				detailDto.setSalePrice(paramSku.getSalePrice());				//C端单个售价
				BigDecimal disCount = new BigDecimal(supplyDto.getDiscountOff());	//供应商折扣（0-100的数字）
				...
				detailDto.setGapPrice(paramSku.getSalePrice().multiply(disCount).divide(new BigDecimal(100)));				//小B提成单价（按供应商折扣计算）
...
//				addNotifyInfo.setSysType(MsgSystemType.ALL.toString());
	...
	@Override
	public Page<OrderDto> findOrderEisPage(FindOrderPage findOrderPage)
			throws TsfaServiceException {
...
//		FindOrderPage findOrderPage = new FindOrderPage();
//		OrderDto param = new OrderDto();
//		param.setOrderNo(orderDetailDto.getOrderNo());
//		List<OrderDto> orderDtos = findOrders(findOrderPage);
//		OrderDto orderDto = orderDtos.get(0);
		
		//消息实体
		MessageDto messageDto = new MessageDto();
//		messageDto.setRecevier(orderDto.getMbrCode());
		messageDto.setRecevier(orderMbrCode);
		messageDto.setcClientCommodity(orderDetailDto.getProductName());
		messageDto.setcClientOrderNo(orderDetailDto.getOrderNo());
		messageService.addMessageByTemplate(messageDto, MessageTemplate.C_SERVICE_BACK_SUCCESS);
	}


	@Override
	public BigDecimal findAmtSum(FindOrderPage findOrderPage)
			throws TsfaServiceException {
	...
	@Override
	public int findOrderPageCount(FindOrderPage findOrderPage)
			throws TsfaServiceException {
	...
	@Override
	public OrderDto findOrderByOrderNo(String orderNo)
			throws TsfaServiceException {
	...
	@Override
	public Integer findAmtRank(String loginShopCode)
			throws TsfaServiceException {
		return orderDao.findAmtRank(loginShopCode);
	}
	...
	public void autoReceipt() throws TsfaServiceException{
		...
		if (orderDtos != null && orderDtos.size()>0) {
			...
			Integer expire_receipt=-Integer.valueOf(Global.getConfig("expire.member_order_receipt"));
			while (localIterator1.hasNext()) {
				OrderDto orderDto = localIterator1.next();
					if(orderDto.getShippingTime()!=null && orderDto.getShippingTime().compareTo(DateUtils.addMinutes(new Date(), -expire_receipt))>0){
	...
	public void autoReview() throws TsfaServiceException{
		...
		if (orderDtos != null && orderDtos.size()>0) {
			Iterator<OrderDto> localIterator1 = orderDtos.iterator();
			Integer expire_receipt=-Integer.valueOf(Global.getConfig("expire.member_order_review"));
			while (localIterator1.hasNext()) {
				OrderDto orderDto = localIterator1.next();
					if(orderDto.getUpdateTime()!=null && orderDto.getUpdateTime().compareTo(DateUtils.addMinutes(new Date(), -expire_receipt))>0){
						...
						parmMemberDto.setCode(orderDto.getCode());
	...
	public static void main(String[] args) throws ParseException {
		System.out.println(new BigDecimal(200).compareTo(new BigDecimal(150))<=0);
//		System.out.println(com.lj.base.core.util.DateUtils.parseDate("2017-09-09",com.lj.base.core.util.DateUtils.PATTERN_yyyy_MM_dd).compareTo(DateUtils.addMinutes(new Date(), -10080))>0);
	}


	@Override
	public List<Map<String, Object>> findOrderGroupStatus(
			FindOrderPage findOrderPage) throws TsfaServiceException {
